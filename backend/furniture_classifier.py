...
You are an expert furniture classification agent. Based on the vision analysis, classify this furniture item for marketplace listing with ALL details optimized by AI.
...
- Detected Features: {vision_result.get('visual_details', {}).get('unique_features', [])}
- Condition: {vision_result.get('visual_details', {}).get('condition_visible', 'Good')}
- Image Quality: {vision_result.get('image_quality', 'Unknown')}

Use AI to determine the BEST classification for maximum marketplace success:

1. **CATEGORY**: Main furniture category
2. **SUBCATEGORY**: Specific furniture type (be precise)
3. **MATERIAL**: Exact material description
4. **STYLE**: Design style that appeals to buyers
5. **TARGET_ROOM**: Where this furniture belongs
6. **KEY_FEATURES**: Top 5 most marketable features (AI-generated)
7. **FACEBOOK_CATEGORY**: Most accurate Facebook Marketplace category
8. **SEARCH_KEYWORDS**: Top 5 keywords buyers would search for
9. **CONDITION_ASSESSMENT**: Detailed condition analysis
10. **MARKET_APPEAL**: What makes this piece attractive to buyers

Provide detailed classification as JSON:
{{
    "category": "Chair",
    "subcategory": "Ergonomic Office Chair",
    "material": "Premium Mesh Fabric with Steel Frame",
    "style": "Modern Professional",
    "target_room": "Home Office",
    "key_features": ["Ergonomic lumbar support", "Adjustable height mechanism", "360-degree swivel", "Breathable mesh backing", "Professional appearance"],
    "facebook_category": "Home & Garden//Furniture//Chairs",
    "search_keywords": ["ergonomic office chair", "mesh office chair", "adjustable desk chair", "professional chair", "work chair"],
    "condition_assessment": "Excellent condition with minimal wear, fully functional",
    "market_appeal": "Perfect for remote workers, home office setups, students",
    "classification_confidence": 0.85,
    "ai_reasoning": "Premium office chair with excellent ergonomic features, high market demand"
}}
    ...
    def _pricing_node(self, state: FurnitureAnalysisState) -> FurnitureAnalysisState:
        ...
        for classification_result in state["classification_results"]:
            try:
                # Pricing prompt
                pricing_prompt = f"""
You are an expert furniture pricing agent for Facebook Marketplace. Calculate the optimal pricing strategy for maximum sales success.
...
- Key Features: {classification_result.get('key_features', [])}
- Condition Assessment: {classification_result.get('condition_assessment', 'Good condition')}
- Market Appeal: {classification_result.get('market_appeal', 'General appeal')}
- AI Reasoning: {classification_result.get('ai_reasoning', 'Standard furniture piece')}

Use AI to determine OPTIMAL pricing strategy:

1. **SUGGESTED_PRICE**: Best price for quick sale while maximizing value
2. **PRICE_RANGE**: Minimum and maximum reasonable prices
3. **PRICING_STRATEGY**: Quick sale vs. maximum profit approach
4. **MARKET_COMPARISON**: Analysis of similar items in current market
5. **VALUE_PROPOSITION**: Why this price is fair/attractive
6. **NEGOTIATION_ROOM**: How much wiggle room for price negotiation
7. **SEASONAL_FACTORS**: Any seasonal pricing considerations

Provide comprehensive pricing analysis as JSON:
{{
    "suggested_price": 125,
    "price_range": {{"min": 100, "max": 150}},
    "pricing_strategy": "Competitive pricing for quick sale",
    "market_comparison": "Similar ergonomic chairs: $100-180, this is priced competitively",
    "value_proposition": "Excellent condition, premium features, priced below retail",
    "negotiation_room": 15,
    "seasonal_factors": "High demand for home office furniture",
    "pricing_confidence": 0.8,
    "pricing_reasoning": "Based on condition, features, and current market demand for office furniture"
}}
        ...
        for group in state["furniture_groups"]:
            try:
                ...
                listing = {
                    ...
                    "condition": listing_data.get("optimized_condition", self._map_condition(furniture_info["condition"])),
                    "description": listing_data.get("description", f"Quality {furniture_info['ai_description'].lower()} in good condition."),
                    "category": listing_data.get("optimized_category", primary_item["classification"].get("facebook_category", "Home & Garden//Furniture")),
                    "confidence": group["avg_confidence"],
                    "images": images,
                    "analysis_source": "LANGGRAPH_WORKFLOW",
                    "processing_time": 0,  # Will be set later
                    # Additional AI-generated metadata
                    "search_keywords": listing_data.get("search_keywords", []),
                    "selling_points": listing_data.get("selling_points", []),
                    "target_buyer": listing_data.get("target_buyer", "General buyers"),
                    "pricing_justification": listing_data.get("pricing_justification", "Market rate"),
                    "condition_reasoning": listing_data.get("condition_reasoning", "Standard assessment"),
                    "langgraph_data": {
                        "group_info": group,
    ...
    def _prepare_image_data(self, group: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        for i, item in enumerate(group["all_items"]):
            ...
            images.append({
                ...
                "processed_url": f"/processed/processed_{filename}"
            })
